...
public class FlickerFetcher {
    ...
    private static final String API_KEY =
            "5aa7aa4b9d1ce439c1682b9cc772d150";
...
//    public List<GalleryItem> fetchItems() {
    ...
    public List<GalleryItem> searchPhotos(String
                                                  query) {
    ...
    downloadGalleryItems(String url) {
...
//                            .build().toString();
    ...
    private String buildUrl(String method, String
            query) {
        Uri.Builder uriBuilder = ENDPOINT.buildUpon()
                .appendQueryParameter("method",
                        method);
        if (method.equals(SEARCH_METHOD)) {
            uriBuilder.appendQueryParameter("text",
                    query);
        }
        return uriBuilder.build().toString();
    }

    private void parseItems(List<GalleryItem> items,
                            JSONObject jsonBody)
        ...
        for (int i = 0; i < photoJsonArray.length();
             i++) {
